...
IMPLEMENT_SINGLETON(CSkillFailFont)

CSkillFailFont::CSkillFailFont(void)
...
HRESULT CSkillFailFont::Initialize(void)
{
...
HRESULT CSkillFailFont::Add_Component(void)
{
	...
	pComponent = m_pBufferCom = (Engine::CVIBuffer*)Engine::Clone_Resource(RESOURCE_STATIC, L"Buffer_SkillFailFont");
	if(NULL == pComponent)
		return E_FAIL;
	m_mapComponent.insert(MAPCOMPONENT::value_type(L"Buffer_HeartBeatUI", pComponent));

	// For.Texture Component
	pComponent = m_pTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STATIC, L"Texture_SkillFailFont");
	if(NULL == pComponent)
		return E_FAIL;
	m_mapComponent.insert(MAPCOMPONENT::value_type(L"Texture_SkillFailFont", pComponent));

	return S_OK;
}

_int CSkillFailFont::Update(const _float& fTimeDelta)
{
	if(m_bRender)
	{
		if(m_bAlphaCheck)
...
void CSkillFailFont::Render(void)
{
	if(NULL == m_pEffect)
		return;
...
_ulong CSkillFailFont::Release(void)
{
