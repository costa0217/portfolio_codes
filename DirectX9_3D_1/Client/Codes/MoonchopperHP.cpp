...
IMPLEMENT_SINGLETON(CMoonchopperHP)

CMoonchopperHP::CMoonchopperHP(void)
...
HRESULT CMoonchopperHP::Initialize(void)
{
...
HRESULT CMoonchopperHP::Add_Component(void)
{
	...
	pComponent = m_pBufferCom[HPFront] = (Engine::CVIBuffer*)Engine::Clone_Resource(RESOURCE_STATIC, L"Buffer_BossFrontMoonchopper");
	if(NULL == pComponent)
		return E_FAIL;
	m_mapComponent.insert(MAPCOMPONENT::value_type(L"Buffer_BossFrontMoonchopper", pComponent));
...
_int CMoonchopperHP::Update(const _float& fTimeDelta)
{
...
void CMoonchopperHP::Render(void)
{
	if(NULL == m_pEffect[0])
		return;
...
void CMoonchopperHP::SetpMonster(CMonster* pMonster)
{
	m_pMonster	= pMonster;
...
_ulong CMoonchopperHP::Release(void)
{
