...
IMPLEMENT_SINGLETON(CBloodLordHP)

CBloodLordHP::CBloodLordHP(void)
...
HRESULT CBloodLordHP::Initialize(void)
{
...
HRESULT CBloodLordHP::Add_Component(void)
{
	...
	pComponent = m_pBufferCom[HPFront] = (Engine::CVIBuffer*)Engine::Clone_Resource(RESOURCE_STATIC, L"Buffer_BossFrontBloodLord");
	if(NULL == pComponent)
		return E_FAIL;
	m_mapComponent.insert(MAPCOMPONENT::value_type(L"Buffer_BossFrontBloodLord", pComponent));
...
_int CBloodLordHP::Update(const _float& fTimeDelta)
{
...
void CBloodLordHP::SetpMonster(CMonster* pMonster)
{
	m_pMonster	= pMonster;
...
_ulong CBloodLordHP::Release(void)
{
