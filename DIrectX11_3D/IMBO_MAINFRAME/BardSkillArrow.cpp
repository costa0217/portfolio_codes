...
CBardSkillArrow::CBardSkillArrow(string name, tag t, bool bStrong)
	:CGameObject(name, t)
{
	...
	m_pArrowTrail->SetTexName(CString("BardTrail01"));

	m_bStrong = bStrong;
}

CBardSkillArrow ::~CBardSkillArrow()
{
	if (m_pArrowTrail)
		delete m_pArrowTrail;
}

void CBardSkillArrow::InitData()
{
...
void CBardSkillArrow::DisappearSkill()
{
...
void CBardSkillArrow::SetActive(bool b)
{
	/*m_bActive = b;
	if (false == m_bActive)
	{
...
void CBardSkillArrow::Initialize()
{
	XMStoreFloat4x4(&m_xmf4x4World, XMMatrixIdentity());
	// 크기 조정
...
void CBardSkillArrow::Animate(float fTimeElapsed)
{
	...
	if (true == m_bStrong)
	{
		...
		if (m_fTraceTime > 0.2f) {
			CEffectMgr::GetInstance()->Play_Effect(L"Bard_Arrow_Trace", XMVectorSet(m_xmf3Position.x, m_xmf3Position.y, m_xmf3Position.z, 1.f),
...
void CBardSkillArrow::RegistToContainer()
{
