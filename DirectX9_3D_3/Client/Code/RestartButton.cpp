...
CRestartButton::CRestartButton(LPDIRECT3DDEVICE9 pDevice)
...
CRestartButton::~CRestartButton(void)
{
	Release();
}

HRESULT CRestartButton::Initialize(void)
{
	FAILED_CHECK(AddComponent());

	m_pInfo->m_vPos = D3DXVECTOR3(0.f, -0.78f, 0.f);
...
int CRestartButton::Update(void)
{
	MouseOver();
...
void CRestartButton::Render(void)
{
...
CRestartButton* CRestartButton::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CRestartButton* pObj = new CRestartButton(pDevice);
...
HRESULT CRestartButton::AddComponent(void)
{
	...
	//Texture
	pComponent = m_pResourceMgr->CloneResource(Engine::RESOURCE_DYNAMIC, L"Texture_Restart");
...
void CRestartButton::Release(void)
{
...
void CRestartButton::TransformMatrix(void)
{
...
void CRestartButton::MouseOver(void)
{
	...
	if(vMousePos.x >= 345 && vMousePos.x <=416 
		&&vMousePos.y >=456 && vMousePos.y <=533)
	{
		m_iButton = 1;
		m_pInfo->m_vPos = D3DXVECTOR3(0.f, -0.80f, 0.f);
...
bool CRestartButton::GetBool(void)
{
	return m_Restart;
}