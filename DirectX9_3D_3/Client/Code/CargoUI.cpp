...
CCargoUI::CCargoUI(LPDIRECT3DDEVICE9 pDevice)
...
CCargoUI::~CCargoUI(void)
{
	Release();
}

HRESULT CCargoUI::Initialize(void)
{
...
void CCargoUI::Render(void)
{
	...
	wsprintf(m_szMission, L"%d / %d", int(HangCount), int(m_Mission));
...
CCargoUI* CCargoUI::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CCargoUI* pObj = new CCargoUI(pDevice);
...
HRESULT CCargoUI::AddComponent(void)
{
	...
	//Texture
	pComponent = m_pResourceMgr->CloneResource(Engine::RESOURCE_DYNAMIC, L"Texture_Cargo");
...
void CCargoUI::TransformMatrix(void)
{
...
void CCargoUI::SetPos(const D3DXVECTOR3& rPos)
{
	m_pInfo->m_vPos = rPos;
}
