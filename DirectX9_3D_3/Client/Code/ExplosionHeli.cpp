...
CExplosionHeli::CExplosionHeli( LPDIRECT3DDEVICE9 pDevice )
:CEffect(pDevice)
, m_fFrameCnt(0.f)
, m_fFrameMax(0.f)
{
...
CExplosionHeli::~CExplosionHeli( void )
{	
	Release();
}

CExplosionHeli* CExplosionHeli::Create( LPDIRECT3DDEVICE9 pDevice )
{
	CExplosionHeli* pObj = new CExplosionHeli(pDevice);
...
HRESULT CExplosionHeli::Initialize( void )
{
...
int CExplosionHeli::Update( void )
{
...
void CExplosionHeli::Render( void )
{
...
void CExplosionHeli::Move( void )
{

}

HRESULT CExplosionHeli::AddComponent( void )
{
	...
	((Engine::CTransform*)pComponent)->SetScale(D3DXVECTOR3(fRand+30.f,fRand+30.f,fRand+30.f));
	m_pInfo = (Engine::CTransform*)pComponent; 
	NULL_CHECK_RETURN(m_pInfo, E_FAIL);
	m_mapComponent.insert(MAPCOMPONENT::value_type(L"Transform", pComponent));

	//Texture
	pComponent = m_pResourceMgr->CloneResource(Engine::RESOURCE_DYNAMIC, L"Texture_HeliExplo");
