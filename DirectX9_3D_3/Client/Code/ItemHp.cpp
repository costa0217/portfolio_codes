...
CItemHp::CItemHp(LPDIRECT3DDEVICE9 pDevice)
...
, m_pResource(NULL)
, m_dwVtxCnt(0)
, m_vPos(D3DXVECTOR3(1200.f, 10.f, 118.f))
...
CItemHp::~CItemHp(void)
{
	Release();
}

HRESULT CItemHp::Initialize(void)
{
	AddComponent();
...
int CItemHp::Update(void)
{
	...
	m_fPlayerHp = m_pPlayerObserver->GetHp();
	...
	{
		((CPlayer*)m_pPlayer)->SetHP(m_fPlayerHp + 50.f);
...
void CItemHp::TransformMatrix(void)
{
...
CItemHp* CItemHp::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CItemHp* pObj = new CItemHp(pDevice);
...
void CItemHp::AddComponent(void)
{
	m_pResource	= m_pResourceMgr->CloneResource(Engine::RESOURCE_DYNAMIC, L"Texture_ItemHp");	
...
void CItemHp::SetPos(const D3DXVECTOR3& rPos)
{
	m_vPos	= rPos;	
}

void CItemHp::SetScale(const D3DXVECTOR3& rScale)
{
	m_vScale	= rScale;
}

void CItemHp::SetAngle(float fAngle[3])
{
	for(int i = 0; i < Engine::ANGLE_END; ++i)
		m_fAngle[i] = fAngle[i];
}

void CItemHp::GetPos(D3DXVECTOR3* pPos)
{
	*pPos = m_vPos;
}

void CItemHp::GetScale(D3DXVECTOR3* pScale)
{
	*pScale = m_vScale;
}

void CItemHp::GetAngle(float* fAngle)
{
	for(int i = 0; i < Engine::ANGLE_END; ++i)
		fAngle[i] = m_fAngle[i];
}

void CItemHp::SetWorldMatrix(const D3DXMATRIX& rMatrix)
{
	m_matWorld = rMatrix;
}

D3DXMATRIX CItemHp::GetWorldMatrix(void)
{
	return m_matWorld;
}

void CItemHp::SetPlayer(Engine::CGameObject* pPlayer)
{
	m_pPlayer = pPlayer;
}
