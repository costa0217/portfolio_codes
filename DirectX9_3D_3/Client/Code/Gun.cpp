...
CGun::CGun( LPDIRECT3DDEVICE9 pDevice )
:CBarrel(pDevice)
{
...
void CGun::Render( void )
{
	...
	m_pResourceMgr->SetVertexInfo(Engine::RESOURCE_DYNAMIC, L"Buffer_Gun", m_pConvertVertex);
	m_pDevice->SetTexture(0, NULL);
	m_pBuffer->Render();

	m_pResourceMgr->SetVertexInfo(Engine::RESOURCE_DYNAMIC, L"Buffer_Gun", m_pVertex);
}	

CGun* CGun::Create( LPDIRECT3DDEVICE9 pDevice )
{	
	CGun* pObj = new CGun(pDevice);
...
HRESULT CGun::AddComponent( void )
{
