...
CMultiBullet::CMultiBullet(LPDIRECT3DDEVICE9 pDevice)
:CBullet(pDevice)
{

}

CMultiBullet::~CMultiBullet(void)
{
	Release();
}

HRESULT CMultiBullet::Initialize(void)
{	
	m_eBulletName	= BUL_MULTI;
	m_fDamage		= 4.f;
	...
	m_pResourceMgr->GetVertexInfo(Engine::RESOURCE_DYNAMIC, L"Buffer_MultiBullet", m_pVertex);
...
void CMultiBullet::Render(void)
{
	m_pDevice->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);
	m_pResourceMgr->SetVertexInfo(Engine::RESOURCE_DYNAMIC, L"Buffer_MultiBullet", m_pConvertVertex);
	m_pDevice->SetTexture(0, NULL);
	m_pBuffer->Render();

	m_pResourceMgr->SetVertexInfo(Engine::RESOURCE_DYNAMIC, L"Buffer_MultiBullet", m_pVertex);
}

CMultiBullet* CMultiBullet::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CMultiBullet* pObj = new CMultiBullet(pDevice);

	return pObj;
}

HRESULT CMultiBullet::AddComponent(void)
{
	...
	//Buffer
	pComponent = m_pResourceMgr->CloneResource(Engine::RESOURCE_DYNAMIC, L"Buffer_MultiBullet");
