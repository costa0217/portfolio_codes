...
CWireStomach::CWireStomach( LPDIRECT3DDEVICE9 pDevice )
: CWire(pDevice)
{

}

CWireStomach::~CWireStomach( void )
{
	Release();
}

HRESULT CWireStomach::Initialize( void )
{
	...
	m_fCenterX  = 30.f; 
	m_fCenterY  = 50.f;
	m_fStickLength = m_fStick_L*20;
	///////

	return S_OK;
}

int CWireStomach::Update( void )
{
	...
	if(m_bWireMove)
	{
		...
		//if(m_fMoveTime > 0.1f)
		{
			m_bWireMove = false;
			m_fMoveTime = 0.f;
			m_WireTail->Set_T_and_V(0.f,m_fHeliSpeed);
			m_WireTail->SetbWireMove(true);
			m_WireTail->SetHeliSpeed(m_fHeliSpeed);
		}
	}
...
void CWireStomach::TransformMatrix( void )
{
	...
	for(size_t i = 0; i < m_dwVtxCnt; ++i)
	{
		...
		else if(i == 2)
		{
			...
			m_WireTail->SetIndex_1(m_pInfo->m_vPos + D3DXVECTOR3(0.f,-8.7f,0.f));
		}
		else if(i == 3)
		{
			m_pInfo->m_vPos.x = m_fUpdatedX-m_fGap + fMoveValue;
			...
			m_WireTail->SetIndex_0(m_pInfo->m_vPos + D3DXVECTOR3(0.f,-8.7f,0.f));
		}
...
CWireStomach* CWireStomach::Create( LPDIRECT3DDEVICE9 pDevice )
{
	CWireStomach* pObj = new CWireStomach(pDevice);
...
void CWireStomach::CreateWireTail()
{
	m_WireTail = CWireTail::Create(m_pDevice);
	m_WireTailList->push_back(m_WireTail);
}
