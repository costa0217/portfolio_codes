...
CGatlingButton::CGatlingButton(LPDIRECT3DDEVICE9 pDevice)
...
CGatlingButton::~CGatlingButton(void)
{
	Release();
}

HRESULT CGatlingButton::Initialize(void)
{
	FAILED_CHECK(AddComponent());

	m_pInfo->m_vPos		= D3DXVECTOR3(0.30f, -0.93f, 0.f);
...
int CGatlingButton::Update(void)
{
	MouseOver();
...
void CGatlingButton::Render(void)
{
...
CGatlingButton* CGatlingButton::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CGatlingButton* pObj = new CGatlingButton(pDevice);
...
HRESULT CGatlingButton::AddComponent(void)
{
	...
	//Texture
	pComponent = m_pResourceMgr->CloneResource(Engine::RESOURCE_DYNAMIC, L"Texture_GatlingGun");
...
void CGatlingButton::Release(void)
{
...
void CGatlingButton::TransformMatrix(void)
{
...
void CGatlingButton::MouseOver(void)
{
	...
	if(vMousePos.x >= 501 && vMousePos.x <=565 
		&&vMousePos.y >=516 && vMousePos.y <=573)
	{
		m_iButton = 1;
		m_pInfo->m_vPos		= D3DXVECTOR3(0.30f, -1.f, 0.f);
	...
	else
	{
		...
		if(m_bClick)
		{
			...
			MAPOBJLIST::iterator	iter3 = m_pMapObjlist->find(L"ZLancherButton");
			MAPOBJLIST::iterator	iter4 = m_pMapObjlist->find(L"Player");

			((CWeaponStatus*)iter->second)->ChangeImage(1);

			((CGunButton*)iter3->second)->SetClick(false);
			((CGunButton*)iter3->second)->ChangeImage(0);

			((CLauncherButton*)iter3->second)->SetClick(false);
			((CLauncherButton*)iter3->second)->ChangeImage(0);

			((CPlayer*)iter4->second)->WeaponCreate(L"Buffer_GatlingGun", WEAPON_GATLELING);

		}
		m_pInfo->m_vPos		= D3DXVECTOR3(0.30f, -0.99f, 0.f);
	}
...
void CGatlingButton::ChangeImage(int image)
{
	m_iButton = image;
}

void CGatlingButton::SetClick(bool click)
{
	m_bClick = click;
}
