...
CBackButton::CBackButton(LPDIRECT3DDEVICE9 pDevice)
...
CBackButton::~CBackButton(void)
{
	Release();
}

HRESULT CBackButton::Initialize(void)
{
	FAILED_CHECK(AddComponent());

	m_pInfo->m_vPos = D3DXVECTOR3(0.38f, -0.78f, 0.f);
...
CBackButton* CBackButton::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CBackButton* pObj = new CBackButton(pDevice);
...
HRESULT CBackButton::AddComponent(void)
{
	...
	//Texture
	pComponent = m_pResourceMgr->CloneResource(Engine::RESOURCE_DYNAMIC, L"Texture_Back");
...
void CBackButton::TransformMatrix(void)
{
...
void CBackButton::MouseOver(void)
{
	...
	if(vMousePos.x >= 476 && vMousePos.x <=550 
