...
CWeaponStatus::CWeaponStatus(LPDIRECT3DDEVICE9 pDevice)
...
CWeaponStatus::~CWeaponStatus(void)
{
	Release();
}

HRESULT CWeaponStatus::Initialize(void)
{
...
void CWeaponStatus::Render(void)
{
...
CWeaponStatus* CWeaponStatus::Create(LPDIRECT3DDEVICE9 pDevice)
{
	CWeaponStatus* pObj = new CWeaponStatus(pDevice);
...
HRESULT CWeaponStatus::AddComponent(void)
{
	...
	//Texture
	pComponent = m_pResourceMgr->CloneResource(Engine::RESOURCE_DYNAMIC, L"Texture_WeaponStatus");
...
void CWeaponStatus::Release(void)
{
...
void CWeaponStatus::TransformMatrix(void)
{
...
void CWeaponStatus::ChangeImage(int image)
{
	m_iButton = image;
}
