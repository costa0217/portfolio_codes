...
HRESULT CNunu::Initialize(void)
{
	m_pSoldier		= NULL;

	m_eMonsterID	= MON_NUNU;
	m_eMonType		= MTYPE_GROUND;

	m_tStats		= UNITSTATS(L"Nunu", 700, 1, 50, 20); //이름, 체력, 최소댐, 최대댐, 이동속도
	m_iGold			= rand()%50 + 50;
...
int CNunu::Progress(void)
{
	...
	if(m_bBattle && !m_bIntengle)
	{
		...
		if( int(m_tFrame.fFrame) == 10 && !m_bDamage)
		{
