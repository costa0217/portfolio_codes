...
HRESULT CBabySpider::Initialize(void)
{
	m_pSoldier		= NULL;

	m_eMonsterID	= MON_BABYSPIDER;
	m_eMonType		= MTYPE_GROUND;

	m_tStats		= UNITSTATS(L"BabySpider", 100, 1, 25, 25); //이름, 체력, 최소댐, 최대댐, 이동속도
	m_iGold			= rand()%10 + 10;
	...
	D3DXVECTOR3 vFstDir = (*m_pVecTile)[m_pVecRoute->back()]->vPos - m_tInfo.vPos;
	D3DXVec3Normalize(&vFstDir, &vFstDir);
	m_tInfo.vDir	= vFstDir;
	...
	m_MyVecRoute = (*m_pVecRoute); //-------------------------루트받아오기
...
int CBabySpider::Progress(void)		
{
	...
	if( m_bDie )
	{
		if(!m_bDeadImg)
		{
			...
			m_bDeadImg = true;
			CSDevice::GetInstance()->SoundPlay(SD_PANG_DIE, 0);
	...
	if(m_bBattle && !m_bIntengle)
	{
		...
		if( int(m_tFrame.fFrame) == 4 && !m_bDamage)
		{
