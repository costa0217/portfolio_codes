...
HRESULT CShif::Initialize(void)
{
	...
	m_tStats		= UNITSTATS(L"Shif", 20, 1, 20, 30); //이름, 체력, 최소댐, 최대댐, 이동속도
	m_iGold			= rand()%20 + 10;
...
int CShif::Progress(void)	
{
	...
	if( m_bDie )
	{
		if(!m_bDeadImg)
		{
			...
			m_bDeadImg = true;
			CSDevice::GetInstance()->SoundPlay(SD_GOBL_DIE, 0);
	...
	if(m_bBattle && !m_bIntengle)
	{
		...
		if( int(m_tFrame.fFrame) == 5 && !m_bDamage)
		{
