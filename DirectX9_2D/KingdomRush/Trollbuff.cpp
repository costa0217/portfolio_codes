...
HRESULT CTrollbuff::Initialize(void)
{
	m_pSoldier		= NULL;

	m_eMonsterID	= MON_TROLLBUF;
	m_eMonType		= MTYPE_GROUND;

	m_tStats		= UNITSTATS(L"Trollbuff", 60, 3, 20, 25);
	m_iGold			= rand()%20 + 40;
	m_fMaxHP		= (float)m_tStats.iHP;

	m_wstrObjKey	= L"Trollbuff";
...
int CTrollbuff::Progress(void)
{
	...
	if( m_bDie )
	{
		if(!m_bDeadImg)
		{
			...
			m_bDeadImg = true;
			CSDevice::GetInstance()->SoundPlay(SD_TROL_DIE, 0);
	...
	if(m_bBattle && !m_bIntengle)
	{
		...
		if( int(m_tFrame.fFrame) == 9 && !m_bDamage)
		{
...
void CTrollbuff::Render(void)
{
	...
	if(m_bDie)
		fX = pTexture->tImgInfo.Width  / 4.f;
