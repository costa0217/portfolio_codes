...
HRESULT CPang::Initialize(void)
{
	m_pSoldier		= NULL;

	m_eMonsterID	= MON_GOBLIN;
	m_eMonType		= MTYPE_GROUND;

	m_tStats		= UNITSTATS(L"Pang", 200, 1, 15, 50); //이름, 체력, 최소댐, 최대댐, 이동속도
	m_iGold			= rand()%60 + 30;
...
int CPang::Progress(void)
{
	FrameMove(m_fFrameSpd);
	...
	if(m_bBattle && !m_bIntengle)
	{
		...
		if( int(m_tFrame.fFrame) == 6 && !m_bDamage)
		{
