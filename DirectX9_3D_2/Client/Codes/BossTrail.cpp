...
CBossTrail::CBossTrail(LPDIRECT3DDEVICE9 pGraphicDev)
...
CBossTrail::~CBossTrail(void)
{

}

HRESULT CBossTrail::Initialize(void)
{
...
HRESULT CBossTrail::Add_Component(void)
{
	...
	pComponent = m_pBuffer = (Engine::CVIBuffer*)Engine::Clone_Resource(RESOURCE_STAGE, L"Buffer_Trail2");
	if(NULL == pComponent)
		return E_FAIL;
...
_int CBossTrail::Update(const _float& fTimeDelta)
{
...
{
	CBossTrail*		pTrail = new CBossTrail(pGraphicDev);
...
void CBossTrail::Set_ContantTable(void)
{
...
void CBossTrail::Create_trail(const _float& fTimeDelta)
{
	...
	m_vSPos[0] = *((CBoss*)m_pBoss)->Get_StartPos();
	m_vEPos[0] = *((CBoss*)m_pBoss)->Get_EndPos();

	/*m_pVTXInfo[0].vPos = *((CBoss*)m_pBoss)->Get_EndPos();
	m_pVTXInfo[20].vPos = *((CBoss*)m_pBoss)->Get_StartPos();
...
void CBossTrail::Set_TrailVB(void)
{
	for(int i = 0; i < 40; ++i)
	{
		if(i < 20)
...
void CBossTrail::Set_StartPos(_vec3* pvEndPos, _vec3* pvStartPos)
{
	m_pvStartPos = pvStartPos;
	m_pvEndPos = pvEndPos;
}

_ulong CBossTrail::Release(void)
{
