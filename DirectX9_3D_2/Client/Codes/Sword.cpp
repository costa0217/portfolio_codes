...
HRESULT CSword::Initialize(void)
{
	...
	m_vSwordEndPos = _vec3((vMin.x + vMax.x) * 0.5f, vMax.y, (vMin.z + vMax.z) * 0.5f);
...
_int CSword::Update(const _float& fTimeDelta)
{
	...
	{	
		...
		m_pParentMatrix = pPlayerMeshCom->Get_FrameMatrix("BN_Weapon_R");

		m_pPlayerTransCom =  (Engine::CTransform*)Engine::Get_Component(L"Com_Transform", L"GameLogic", L"Player");
		m_pParentWorldMatrix = &m_pPlayerTransCom->m_matWorld;		
	}
