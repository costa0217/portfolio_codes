...
IMPLEMENT_SINGLETON2(CSDevice)

CSDevice::CSDevice(void)
{
}
...
//사운드파일을 읽기 위한함수(경로)
HRESULT CSDevice::LoadWave(TCHAR* pFileName)
{
	HMMIO	hFile;
...
void CSDevice::SoundPlay(int iIndex, DWORD dwFlag)
{
...
void CSDevice::SoundStop(int iIndex)
{
	if(iIndex < 0 || iIndex > (signed)m_vecSoundBuff.size())
		return;
...
bool CSDevice::SoundPlaying(int iIndex)
{
	DWORD	dwStatus = 0;
