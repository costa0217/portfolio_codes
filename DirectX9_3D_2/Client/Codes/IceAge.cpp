...
CIceAge::CIceAge(LPDIRECT3DDEVICE9 pGraphicDev, int iIdx)
...
HRESULT CIceAge::Initialize(void)
{
...
HRESULT CIceAge::Add_Component(void)
{
	...
	// For.Mesh Component
	pComponent = m_pMeshCom = (Engine::CStaticMesh*)Engine::Clone_Resource(RESOURCE_STAGE, L"Mesh_IceAge");
	if(NULL == pComponent)	return E_FAIL;
...
_int CIceAge::Update(const _float& fTimeDelta)
{
	...
	if(m_fAccTime < 4.8f + ((m_iIndex - 1)  * 0.2f))	return 0;
	if(!m_bSound)
	{
		CSDevice::GetInstance()->SoundPlay(SD_BOSSGROUNDSHOCK, 0);
		m_bSound = true;
	}
...
void CIceAge::Render(void)
{
	if(NULL == m_pEffect)
		return;

	if(!m_bSwitch)	return;
	if(m_fAccTime < 4.8f + ((m_iIndex - 1)  * 0.2f))	return ;
...
CIceAge* CIceAge::Create(LPDIRECT3DDEVICE9 pGraphicDev, int iIdx)
{
	CIceAge*		pGameObject = new CIceAge(pGraphicDev, iIdx);
...
void CIceAge::Check_Collision(void)
{
...
void CIceAge::Set_ContantTable(void)
{
...
void CIceAge::Set_SettingData(void)
{	
	...
	m_pTransCom->m_vScale = m_pBossTransCom->m_vScale;
...
void CIceAge::Set_Pos(_vec3 vPos)
{
	m_pTransCom->m_vPosition = vPos;
}

void CIceAge::Set_DataInit(void)
{
	m_bSwitch		= false;	
