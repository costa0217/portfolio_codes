...
HRESULT CLogoScene::Ready_Resources(void)
{
	...
	// For.Texture ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	if(FAILED(Engine::Ready_Texture(m_pGraphicDev, RESOURCE_LOGO, L"Texture_Logo", Engine::CTexture::TYPE_NORMAL, L"../Bin/Resource/Texture/LogoScene/Logo%d.tga", 1)))
	{
		MSG_BOX("Texture_Logo Failed");
		return E_FAIL;
	}
	if(FAILED(Engine::Ready_Texture(m_pGraphicDev, RESOURCE_LOGO, L"Texture_TitleName", Engine::CTexture::TYPE_NORMAL, L"../Bin/Resource/Texture/LogoScene/TitleName/Opening%d.tga", 19)))
	{
		MSG_BOX("Texture_TitleName Failed");
		return E_FAIL;
	}
...
HRESULT CLogoScene::Ready_Environment(void)
{
	...
	// For.CLogoBack Instance
	pGameObject = CTitleName::Create(m_pGraphicDev);
	if(NULL == pGameObject)
		return E_FAIL;
	pLayer->Ready_Object(L"TitleName", pGameObject);
