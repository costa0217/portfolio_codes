...
void CAnimationCtrl::Set_AnimationSet(const _uint& iAniIdx)
{
	...
	m_pAniCtrl->KeyTrackEnable(m_dwCurrentTrack, FALSE, m_fAccTime + 0.20f);
	m_pAniCtrl->KeyTrackSpeed(m_dwCurrentTrack, 1.f, m_fAccTime, 0.20f, D3DXTRANSITION_LINEAR);
	m_pAniCtrl->KeyTrackWeight(m_dwCurrentTrack, 0.1f, m_fAccTime, 0.20f, D3DXTRANSITION_LINEAR);

	// 특정 트랙을 활성화시킨다.	
	m_pAniCtrl->KeyTrackEnable(m_dwNewTrack, TRUE, m_fAccTime + 0.20f);	
	m_pAniCtrl->KeyTrackSpeed(m_dwNewTrack, 1.f, m_fAccTime, 0.20f, D3DXTRANSITION_LINEAR);
	m_pAniCtrl->KeyTrackWeight(m_dwNewTrack, 0.9f, m_fAccTime, 0.20f, D3DXTRANSITION_LINEAR);
...
_bool CAnimationCtrl::Check_EndPeriod(_float fFixTime)
{
	...
	if(m_Period - fFixTime < TrackInfo.Position)
		return true;
	return false;
}
