...
public:
	void Add_RenderGroup(RENDERTYPE eType, Engine::CGameObject* pGameObject, _float fViewZ) 
	{
		...
		if(eType == RENDER_UI)	
		{
			m_mapUI.insert(MAPUI::value_type(fViewZ, pGameObject));		
		}
	}
...
private:
	...
	multimap<float, Engine::CGameObject*, greater<float>>			m_mapUI;
	typedef multimap<float, Engine::CGameObject*, greater<float>>	MAPUI;
private:
